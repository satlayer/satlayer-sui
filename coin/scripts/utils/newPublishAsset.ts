...
import getTxHex from "../../../core/scripts/utils/getTxHex";
import getExecStuff from "../../../core/scripts/utils/execStuff";

// Helper delay function
const sleep = (ms: number) => new Promise(resolve => setTimeout(resolve, ms));

const publishNewAsset = async ({moduleName, name, symbol, description, icon_url, decimal, useMultiSig}: {
  moduleName: string,
  name: string,
  symbol: string,
  description: string,
  icon_url: string,
  decimal: number,
  useMultiSig: boolean, // return tx hex if true, else execute the transaction
}) => {
  let packageId = '';
  let CoinMetadata = '';
  let UpgradeCap = '';
  ...
  try {
    ...
    if (useMultiSig) {
      const txHex = await getTxHex({
        tx,
        client,
        gasBudget: 100000000 // gas is higher for new coin deployment
      });
      return {txHex}
    }
