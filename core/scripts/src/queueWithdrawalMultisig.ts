...
const WITHDRAW_AMOUNT = 100_000_000; // Replace with desired withdraw amount, in smallest denomination (e.g., 1 satYBTC.B = 100_000_000)
...
async function queueWithdrawal() {
    ...
    const coinToWithdraw = coinWithBalance({
      type: RECEIPT_COIN_TYPE,
      balance: WITHDRAW_AMOUNT,
    });

    tx.moveCall({
        target: `${PACKAGE_ID}::satlayer_pool::queue_withdrawal`,
        arguments: [
