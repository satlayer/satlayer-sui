...
const DEPOSIT_AMOUNT = 100_000_000; // Replace with desired deposit amount, in smallest denomination (e.g., 1 YBTC.B = 100_000_000)
...
async function depositFor() {
    ...
    const coinToDeposit = coinWithBalance({
      type: DEPOSIT_COIN_TYPE,
      balance: DEPOSIT_AMOUNT,
    });

    const receiptCoin = tx.moveCall({
        target: `${PACKAGE_ID}::satlayer_pool::deposit_for`,
        arguments: [
    ...
    tx.transferObjects([receiptCoin], SIGNER_ADDRESS)
